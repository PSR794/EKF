...
#mean and covariances matricesaaaa
...
for i in range(1,501):
    ...
    #change
    control=np.zeros([2,1],dtype=float)
    control[0][0]=vel[i-1]
    control[1][0]=w[i-1]*dt

    #prediction mean
    x1=x1+(g@control) #prediction first step (gives us ut')
 
    x1[2][0]=wraptopi(x1[2][0])
    
    #jacobian 1 + I(wrt x,y,theta) #change
    G=np.zeros([3,3],dtype=float)
    G[1][1]=G[0][0]=G[2][2]=1
    G[0][2]=(np.sin(v)*(-1)*dt*vel[i-1])
    G[1][2]=(np.cos(v)*dt*vel[i-1])

    #jacobian 2(wrt v,w) #change
...
#        print('sd',angle)
        ...
        #(jacobian of h) H, 3 variables and 2 functions (wrt x,y,theta) cyrill model to be applied
